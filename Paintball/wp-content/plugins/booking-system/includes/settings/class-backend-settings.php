...
*/
    ...
    if (!class_exists('DOPBSPBackEndSettings')){
		...
		global $DOT;
                ...
                $id = $is_ajax ? $DOT->post('id', 'int'):$DOT->sanitize($args['id'], 'int');
                $settings_type = $is_ajax ? $DOT->post('settings_type'):$DOT->sanitize($args['settings_type']);
                $key = $is_ajax ? $DOT->post('key'):$DOT->sanitize($args['key']);
                $post_value = $key == 'hours_definitions'
                        ? ($is_ajax ? $DOT->post('value', false):$DOT->sanitize($args['value'], false))
                        : ($is_ajax ? $DOT->post('value'):$DOT->sanitize($args['value']));
