 ...
 * @return boolean
 */
function technocy_is_product_archive() {
 ...
 */
function technocy_product_get_image($product) {
 ...
 * @param $product WC_Product
 */
function technocy_product_get_price_html($product) {
 ...
 *
 */
function technocy_get_previous_product($in_same_term = false, $excluded_terms = '', $taxonomy = 'product_cat') {
    $product = new Technocy_WooCommerce_Adjacent_Products($in_same_term, $excluded_terms, $taxonomy, true);
 ...
 *
 */
function technocy_get_next_product($in_same_term = false, $excluded_terms = '', $taxonomy = 'product_cat') {
    $product = new Technocy_WooCommerce_Adjacent_Products($in_same_term, $excluded_terms, $taxonomy);
    return $product->get_product();
}


function technocy_is_woocommerce_extension_activated($extension = 'WC_Bookings') {
...
function technocy_woocommerce_pagination_args($args) {
    $args['prev_text'] = '<i class="technocy-icon technocy-icon-angle-left"></i><span>' . esc_html__('Previous', 'technocy') . '</span>';
    $args['next_text'] = '<span>' . esc_html__('Next', 'technocy') . '</span><i class="technocy-icon technocy-icon-angle-right"></i>';
    return $args;
}

add_filter('woocommerce_pagination_args', 'technocy_woocommerce_pagination_args', 10, 1);


function technocy_unsupported_theme_remove_review_tab($tabs) {
    unset($tabs['reviews']);
    return $tabs;
}
 ...
 * @return bool
 */
function technocy_woocommerce_is_deal_product($product) {
...
if (!function_exists('technocy_ajax_search_products')) {
    function technocy_ajax_search_products() {
        global $woocommerce;
        ...
        $args = array(
            's'                   => apply_filters('technocy_ajax_search_products_search_query', $search_keyword),
            'post_type'           => 'product',
            'post_status'         => 'publish',
            'ignore_sticky_posts' => 1,
            'orderby'             => $ordering_args['orderby'],
            'order'               => $ordering_args['order'],
            'posts_per_page'      => apply_filters('technocy_ajax_search_products_posts_per_page', 8),
        ...
        if (!empty($products)) {
            foreach ($products as $post) {
                ...
                $suggestions[] = apply_filters('technocy_suggestion', array(
                    'id'    => $product->get_id(),
            ...
            $suggestions[] = array(
                'id'    => -1,
                'value' => esc_html__('No results', 'technocy'),
                'url'   => '',
            );
        }
        wp_reset_postdata();
...
add_action('wp_ajax_technocy_ajax_search_products', 'technocy_ajax_search_products');
add_action('wp_ajax_nopriv_technocy_ajax_search_products', 'technocy_ajax_search_products');
...
if ( ! function_exists( 'wvs_taxonomy_meta_fields' ) ){
    function wvs_taxonomy_meta_fields( $field_id = false ) {
        ...
        $fields['color'] = array(
            array(
                'label' => esc_html__( 'Color', 'technocy' ), // <label>
                'desc'  => esc_html__( 'Choose a color', 'technocy' ), // description
        ...
        $fields['image'] = array(
            array(
                'label' => esc_html__( 'Image', 'technocy' ), // <label>
                'desc'  => esc_html__( 'Choose an Image', 'technocy' ), // description
