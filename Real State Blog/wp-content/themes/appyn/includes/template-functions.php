		...
		<span class="rating">
			...
			if($calificar == 1){ ?><span class="rating-average"><b><?php echo $count_rating['average']; ?></b>/5</span>
				...
				if($count_rating['users'] > 0) {
					echo __( 'Votos', 'appyn' ).': <span>'.number_format($count_rating['users'], 0, ',', ',').'</span>';
				} else {
...
function px_content_filter($content){
	if( is_amp_px() ) {
        ...
        if ($matches) {
            ...
            $re = '/align="(.*?)"/m';
            $images[$m[0]] = preg_replace($re, '', $images[$m[0]]);
...
function cover_header() {
	...
	$im = $arrayimgs[rand(0,(count($arrayimgs) - 1))];
...
function px_upload_image( $datos, $post_id, $url_app ) {
...
function px_show_first_dl() {
	...
	elseif( $datos_download['option'] == "links" ) {
		return $datos_download['links_options'][0]['link'];
	}
}
