 ...
 * @extends WC_Widget
 */
class Technocy_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'technocy');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'technocy');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'technocy'),
				'label' => esc_html__('Title', 'technocy'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'technocy'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'technocy'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'technocy'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'technocy'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'technocy'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'technocy'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'technocy'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Technocy_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'technocy');
			...
			echo '<ul class="product-brands" id="technocy-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Technocy_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'technocy'),
		'singular_name'              => esc_html__('Brands', 'technocy'),
		'menu_name'                  => esc_html__('Brands', 'technocy'),
		'all_items'                  => esc_html__('All Brands', 'technocy'),
		'parent_item'                => esc_html__('Parent Brand', 'technocy'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'technocy'),
		'new_item_name'              => esc_html__('New Brand Name', 'technocy'),
		'add_new_item'               => esc_html__('Add New Brands', 'technocy'),
		'edit_item'                  => esc_html__('Edit Brand', 'technocy'),
		'update_item'                => esc_html__('Update Brand', 'technocy'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'technocy'),
		'search_items'               => esc_html__('Search Brands', 'technocy'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'technocy'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'technocy'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
