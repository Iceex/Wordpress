...
class EPS {
    ...
    private function import_process() {
        ...
        if( isset($bot['response']) ) {
            $output['info'] = $bot['response'];
            echo json_encode($output);
            exit;
        }

        if( isset($bot['error_web']) ) {
            $output['info'] = __( 'Error: Este dominio no está habilitado.', 'appyn' ).' <a href="https://themespixel.net/en/docs/appyn/api-key/" target="_blank">'.__( 'Más información', 'appyn' ).'</a>';
            echo json_encode($output);
            exit;
        }
        ...
        if( !isset( $bot['app'] ) ) {
            $output['info'] = __( 'Error: No se pudo obtener los datos de esta app.', 'appyn' );
            echo json_encode($output);
            exit;
        }
    ...
    public function updatePost( $post_id ) {
        ...
        $this->url_app = get_datos_info('consiguelo', false, $this->post_id);;
    ...
    private function after_process( $type = 'create' ) {
        ...
        if( $type == 'update' )
            if( ! in_array('app_ico', $this->dca) ) {
                ...
                $attach_id = px_upload_image( $this->info_app, $this->post_id, $this->url_app );
            }
    ...
    private function processAPK() {
        ...
        if( $this->bot_info['apk'] ) {
            ...
            $this->output['apk_info'] = array(
                ...
                'total_size' 	  => $this->bot_info['total_size'],
            );
